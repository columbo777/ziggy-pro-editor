...
namespace ProUpgradeEditor.UI
{
    public partial class MainForm : Form
    {
        ...
        public void HandlePackageDrop(DragEventArgs e)
        {
            try
            {
                if (e.Data.GetDataPresent(DataFormats.FileDrop, false) == true)
                {
                    ...
                    ImportPackageFiles(packageFiles);
                }
            }
            catch { }
        }

        public void ImportPackageFiles(string[] packageFiles)
        {

            if (packageFiles != null && packageFiles.Length > 0)
            {
                var outputFolder = textBoxDefaultMidiProFileLocation.Text;
                ...
                {
                    foreach (string packageFileName in packageFiles)
                    {
                        CloseSelectedSong();
                        try
                        {
                            var package = Package.Load(packageFileName);
                            ...
                            if (package != null)
                            {
                                ...
                                {
                                    dir = Path.Combine(dir, Path.GetFileNameWithoutExtension(packageFileName));
                                    ...
                                    try
                                    {
                                        var dtaFiles = ExtractDTAFiles(package, dir);
                                        var midiFiles = ExtractMidiFiles(package, dir);

                                        if (dtaFiles.Any() && midiFiles.Any())
                                        {
                                            foreach (var midiFileName in midiFiles)
                                            {
                                                CreateSongFromFiles(packageFileName, midiFileName, dtaFiles);
                                            }
                                        }
                                    }
                                    catch { }
        ...
        public void CreateSongFromFiles(string packageFileName, string guitar5FileLocation, List<string> dtaFiles)
        {

            if (!string.IsNullOrEmpty(packageFileName) &&
                !string.IsNullOrEmpty(guitar5FileLocation) &&
                dtaFiles.Count>0)
            {
                ...
                try
                {
                    ...
                    if (File.Exists(guitar5FileLocation))
                    {
                        string songID = string.Empty;
                        foreach (var dtaFile in dtaFiles)
                        {
                            var dta = File.ReadAllBytes(dtaFile);
                            if (dta != null)
                            {
                                songID = DTAGetSongID(dta);
                        ...
                        if (!string.IsNullOrEmpty(songID))
                        {
                            var sngs = SongList.Where(x =>
                                !string.IsNullOrEmpty(x.DTASongID) &&
                                string.Compare(x.DTASongID, songID, true) == 0);

                            if (!sngs.Any())
                            {
                                CloseSelectedSong();

                                if (OpenEditorFile(guitar5FileLocation))
                                {
                ...
                {
                    WriteBatchResult("Error : " + guitar5FileLocation);
                    WriteBatchResult(ex.Message);
                }
            }
        }
        ...
        private void RegenDifficultiesClick()
        {
            ...
            if (SelectedSong != null)
            {
                ...
                if (!SaveProCONFile(SelectedSong, !File.Exists(SelectedSong.G6ConFile), true))
                {
        ...
        private void button101_Click(object sender, EventArgs e)
        {
            ...
            var selPack = ShowOpenFileDlg("Select Package", textBoxDefaultCONFileLocation.Text, selCon);
        ...
        private void openPro17ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string file = ShowOpenFileDlg("Open Rock Band 3 Pro Midi File",
                    textBoxDefaultMidiProFileLocation.Text, "");
        ...
        {
            if (Utility.GetArpeggioData1(EditorPro.CurrentDifficulty) == -1)
                return;
        ...
        private void button62_Click(object sender, EventArgs e)
        {
            ...
            {
                textBoxDefaultCONFileLocation.Text = s;
            }
        }

        private void button63_Click(object sender, EventArgs e)
        {
            var s = ShowSelectFolderDlg("Select default Midi 5 folder", textBoxDefaultMidi5FileLocation.Text, "");
            if (s != string.Empty)
            {
                textBoxDefaultMidi5FileLocation.Text = s;
            }
        }

        private void button64_Click(object sender, EventArgs e)
        {
            var s = ShowSelectFolderDlg("Select default Pro Midi folder", textBoxDefaultMidiProFileLocation.Text, "");
            if (s != string.Empty)
            {
                textBoxDefaultMidiProFileLocation.Text = s;
            }
        }
        ...
        private void button65_Click(object sender, EventArgs e)
        {
            ...
            var s = ShowOpenFileDlg("Select midi file",
                textBoxDefaultMidi5FileLocation.Text, path);
            if (!string.IsNullOrEmpty(s))
            {
                textBox21.Text = s;
        ...
        private void button66_Click(object sender, EventArgs e)
        {
            ...
            var s = ShowOpenFileDlg("Select pro midi file",
                textBoxDefaultMidiProFileLocation.Text, path);
            if (!string.IsNullOrEmpty(s))
            {
                textBox22.Text = s;
        ...
        {
            var s = ShowOpenFileDlg("Select pro CON file",
                textBoxDefaultCONFileLocation.Text,
                textBoxSongLibConFile.Text);
            if (!string.IsNullOrEmpty(s))
            {
                textBoxSongLibConFile.Text = s;
        ...
        public void ZoomOut()
        {
            if (!reloading)
            {
                if (Utility.timeScalar > 1)
                {
                    ...
                    if (EditorG5.HScrollValue != EditorPro.HScrollValue &&
                        EditorG5.IsLoaded && EditorPro.IsLoaded)
                    {
                        try { EditorG5.HScrollValue = EditorPro.HScrollValue; }
                        catch { }
                    }
                }
            }
        ...
        private void button91_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(pkPath) || !File.Exists(pkPath))
                {
                    pkPath = ShowOpenFileDlg("Select CON Package To Extract",
                        textBoxDefaultCONFileLocation.Text, pkPath);
                ...
                if (!string.IsNullOrEmpty(pkPath) && File.Exists(pkPath))
                {
                    ...
                    else
                    {
                        ...
                        var sel = ShowSelectFolderDlg("Select Package Output Path", textBoxDefaultCONFileLocation.Text, targetPath);
                        if (!string.IsNullOrEmpty(sel))
                        {
                            ...
                            ExtractPackageContents(sel, pk.RootFolder, filters);
        ...
        private void ShowSelectCompressAllZipFile()
        {
            var sel = ShowSaveFileDlg("Select Output Zip File", "", textBoxCompressAllZipFile.Text);
            if (!string.IsNullOrEmpty(sel))
            {
                textBoxCompressAllZipFile.Text = sel;
            }
        }

        private void button118_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(textBoxCompressAllZipFile.Text))
            {
                ShowSelectCompressAllZipFile();
            }
            ...
            if (!string.IsNullOrEmpty(textBoxCompressAllZipFile.Text))
            {
                try
                {
                    ...
                    {
                        string folderToCompress = textBoxDefaultCONFileLocation.Text;
                        if (string.IsNullOrEmpty(folderToCompress))
                        {
                            folderToCompress =
                                ShowSelectFolderDlg("Select folder to compress", "", "");
                        }
                        
                        if (!string.IsNullOrEmpty(folderToCompress) &&
                            Directory.Exists(folderToCompress))
                        {
                            fileNames = Directory.GetFiles(folderToCompress);
                        }
                    ...
                    else
                    {
                        ...
                        {
                            if (song.IsComplete && !string.IsNullOrEmpty(song.G6ConFile) && File.Exists(song.G6ConFile))
                            {
                                files.Add(song.G6ConFile);
                            }
                        }
                    ...
                    {
                        using (ZipOutputStream s = new ZipOutputStream(File.Create(textBoxCompressAllZipFile.Text)))
                        {
                            s.SetLevel(2);
                            ...
                            foreach (string file in fileNames)
                            {
                                ...
                                entry.Size = File.OpenRead(file).Length;
                        ...
                        OpenExplorerFolder(Path.GetDirectoryName(textBoxCompressAllZipFile.Text));
                    }
                }
        ...
        public bool CopyPowerupsFromG5(bool alltracks)
        {
            ...
            try
            {
                ...
                EditorG5.SetTrack5(EditorG5.GetGuitar5MidiTrack(), GuitarDifficulty.Expert);
                EditorPro.SetTrack6(EditorPro.GetGuitar6MidiTrack(), GuitarDifficulty.Expert);
        ...
        public bool OpenEditorFile(string fileName, byte[] data=null, EditorFileType openFileType = EditorFileType.Any)
        {
            ...
            if ((openFileType & EditorFileType.AnyCon) != 0)
            {
                ...
                if (p != null)
                {
                    ...
                    foreach (var mid in midiFiles)
                    {
                        try
                        {
                            var seq = new Sequence(FileType.Unknown);
                            using (var ms = new MemoryStream(mid.Data))
                            {
                                seq.Load(ms);
                            }
            ...
            if (opened)
            {
                if (track5 != null)
                {
                    ret = EditorG5.SetTrack5(seq5, track5, GuitarDifficulty.Expert);
                    if (ret)
                    {
                        FileNameG5 = fileName;
                        RefreshTracks5();
                ...
                if (track6 != null)
                {
                    ret = EditorPro.SetTrack6(seq6, track6, GuitarDifficulty.Expert);
                    if (ret)
                    {
            ...
            try
            {
                ...
                if (f != null)
                {
                    try
                    {
                        ...
                        {
                            var seq = new Sequence(FileType.Unknown);
                            using (var ms = new MemoryStream(f.Data))
                            {
                                seq.Load(ms);
                            }
            ...
            if (e.Data.GetDataPresent(DataFormats.FileDrop, false) == true)
            {
                ...
                foreach (string file in files)
                {
                    try
                    {
                        ...
                        var seq = new Sequence(FileType.Guitar5, file);

                        var t5 = seq.Tracks.FirstOrDefault(x=> 
                            GuitarTrack.TrackNames5.Contains(x.Name));
                        if(t5 == null && seq.Tracks.Length > 0)
                        {
                            t5 = seq.Tracks[0];
                        }
                        if (t5 != null)
                        {
                            FileNameG5 = file;
                            trackEditorG5.SetTrack5(seq, t5, GetEditorDifficulty());
                        }
                    ...
                    foreach (var m in mcp.Matches)
                    {
                        ...
                        minTick = ReplaceNotes(EditorPro.GuitarTrack, 
                            mcp.OriginalChords6, mcp.DeltaTimeStart,
                                 minTick,  m,
                                m.GetMinTick(), m.GetMaxTick(), out inew, out irep);
        ...
        private void buttonRemoveProGuitarTrainer_Click(object sender, EventArgs e)
        {
            ...
            if (gt != null)
            {
                ...
                ProGuitarTrack.Remove(gt.Start.MidiEvent);
                if (gt.Loopable)
                {
                    ProGuitarTrack.Remove(gt.Norm.MidiEvent);
                }
                ProGuitarTrack.Remove(gt.End.MidiEvent);
            }
            ReloadTrack();
        }
        ...
        {
            if (SelectedProGuitarTrainer != null && EditorPro.IsLoaded)
            {
                var start = textBoxProGuitarTrainerBeginTick.Text.ToInt();
                var end = textBoxProGuitarTrainerEndTick.Text.ToInt();
                if (end > start)
                {
                    SelectedProGuitarTrainer.SetTicks(start, end, checkTrainerLoopableProGuitar.Checked);

                    if (checkTrainerLoopableProGuitar.Checked)
                    {
                        if (SelectedProGuitarTrainer.Norm.MidiEvent == null)
                        {
                            var mb = new MetaTextBuilder();

                            mb.Text = SelectedProGuitarTrainer.NormText;
                            mb.Type = MetaType.Text;
                            mb.Build();
                            ProGuitarTrack.Insert(SelectedProGuitarTrainer.Norm.AbsoluteTicks, mb.Result);
                        }
                    }
                    else
                    {
                        if (SelectedProGuitarTrainer.Norm.MidiEvent != null)
                        {
                            ProGuitarTrack.Remove(SelectedProGuitarTrainer.Norm.MidiEvent);
                            SelectedProGuitarTrainer.Norm.MidiEvent = null;
                        }
                    }
                    ReloadTrack();
        ...
        {
            if (SelectedProBassTrainer != null && EditorPro.IsLoaded)
            {
                var start = textBoxProBassTrainerBeginTick.Text.ToInt();
                var end = textBoxProBassTrainerEndTick.Text.ToInt();
                if (end > start)
                {
                    SelectedProBassTrainer.SetTicks(start, end, checkTrainerLoopableProBass.Checked);
                    if (checkTrainerLoopableProBass.Checked)
                    {
                        if (SelectedProBassTrainer.Norm.MidiEvent == null)
                        {
                            var mb = new MetaTextBuilder();

                            mb.Text = SelectedProBassTrainer.NormText;
                            mb.Type = MetaType.Text;
                            mb.Build();
                            ProGuitarTrack.Insert(SelectedProBassTrainer.Norm.AbsoluteTicks, mb.Result);
                        }
                    }
                    else
                    {
                        if (SelectedProBassTrainer.Norm.MidiEvent != null)
                        {
                            ProGuitarTrack.Remove(SelectedProBassTrainer.Norm.MidiEvent);
                            SelectedProBassTrainer.Norm.MidiEvent = null;
                        }
                    }
        ...
        private void buttonDeleteTextEvent_Click(object sender, EventArgs e)
        {
            if (EditorPro.IsLoaded)
            {
                try
                {
                    ...
                    {
                        ProGuitarTrack.Remove(EditorPro.SelectedTextEvent.MidiEvent);
                    }
                }
                catch { }
                ReloadTrack();
        ...
        public void GenerateTrainers(bool selectedTrackOnly)
        {
            try
            {
                ...
                foreach (var tr in gtList)
                {
                    if (tr != null)
                    {
                        if (tr.FileType == FileType.Pro)
                        {
                            EditorPro.SetTrack6(tr);
                            ReloadTrack();
                            if (!EditorPro.Messages.Trainers.Any())
                            {
                                if (EditorPro.Messages.Solos.Any())
                                {
                                    foreach (var solo in EditorPro.Messages.Solos)
                                    {
                                        CreateTrainer(
                                             GuitarTrack.IsGuitarTrackName(tr.Name) ?
                ...
                EditorPro.SetTrack6(ct, GetEditorDifficulty());
            }
            catch { }
        }
        ...
        public void CopyTextEvents(bool selectedTrackOnly)
        {
            try
            {
                ...
                {
                    var mb = new MetaTextBuilder();
                    foreach (var te5 in G5Track.TextEvents)
                    {
                        if (!ProGuitarTrack.TextEvents.Any(x => x.AbsoluteTicks == te5.AbsoluteTicks && x.Text == te5.Text))
                        {
                            mb.Text = te5.Text;
                            mb.Type = te5.MidiEvent.MetaMessage.MetaType;
                            mb.Build();
                            ProGuitarTrack.Insert(te5.AbsoluteTicks, mb.Result);
                        }
                    }
        ...
        private void midiTrackEditorPro_TrackAdded(PEMidiTrackEditPanel sender, Sequence sequence, Track track, GuitarDifficulty difficulty)
        {
            ...
            if (GuitarTrack.TrackNames5.Contains(track.Name))
            {
                if (InitializeEventsFromG5(true, true, false, track))
                {
                    ReloadTracks();
                }
            }
        ...
        private void Refresh108EventList()
        {
            ...
            try
            {
                if (EditorPro != null && EditorPro.GuitarTrack != null)
                {
                    var handEvents = EditorPro.GuitarTrack.ChanEventsAll.Where(x => 
                        x.Data1 == Utility.HandPositionData1 &&
                        x.Command == ChannelCommand.NoteOn).ToList();

                    foreach (var ev in handEvents)
                    {
                        list108.Items.Add(ev);
                    }
                }
            }
            catch { }
        ...
        {
            var s = ShowOpenFileDlg("Select MP3 file",
                textBoxDefaultCONFileLocation.Text,
                textBoxSongPropertiesMP3Location.Text);
            if (!string.IsNullOrEmpty(s))
            {
                textBoxSongPropertiesMP3Location.Text = s;
