...
namespace Pitch
{
    ...
    public class PitchTracker
    {
        ...
        public IEnumerable<PitchRecord> ProcessBuffer(float[] inBuffer, int sampleCount = 0)
        {
            List<PitchRecord> ret = new List<PitchRecord>();
            if (inBuffer == null)
            ...
            while (samplesProcessed < srcLength)
            {
                ...
                while (m_circularBufferLo.ReadBuffer(m_pitchBufLo, m_curPitchSamplePos, m_pitchBufSize + m_detectOverlapSamples))
                {
                    ...
                    if (detectedPitch > 20.0)
                    {
                        ret.Add(AddPitchRecord(detectedPitch));
                    }
